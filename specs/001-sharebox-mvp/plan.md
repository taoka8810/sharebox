...
### Technology Stack (Fixed Constraints) — Drift Notice

Constitution の Technology Stack 表に「**認証**: Arctic + **Lucia** による Google
OAuth 自前実装」と記載されているが、調査の結果 Lucia ライブラリ本体 (v3) は 2025
年に作者によって正式にメンテナンス終了が宣言されており、代わりに「Arctic +
`@oslojs/crypto` ベースの自前セッション実装」を `lucia-auth.com` のチュートリアル
が公式に推奨している。本 plan はこのチュートリアル準拠の構成を採用する。

これは Constitution の文言を厳密に読めば軽微な逸脱だが:

1. 精神 (「軽量で 1 ユーザーホワイトリストの自前実装」) は完全に維持
2. 採用ライブラリ `@oslojs/*` は元 Lucia 作者によるもので、Lucia v3 内部の抽出
3. 機能差はゼロ、依存は寧ろ縮小

このため Complexity Tracking には記録するが、Constitution の文言更新は PATCH 改訂
として `/speckit.implement` フェーズの開始前に別途行う。
...
|-----------|------------|--------------------------------------|
| Constitution の "Arctic + Lucia" 表記からの軽微な逸脱 (Lucia 本体ではなく `@oslojs/*` を使用) | Lucia v3 が 2025 年にメンテ終了し、`lucia-auth.com` 公式チュートリアルが Arctic + `@oslojs/crypto` ベースの自前セッション実装を推奨している | メンテ終了済みの Lucia v3 を採用すると将来的にセキュリティ修正が得られず、Principle V (品質) に反する |
