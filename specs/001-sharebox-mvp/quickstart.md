...
4. 各クライアントの Authorized redirect URIs を設定:
   ...
   - prod: `https://sharebox-dls.pages.dev/login/google/callback`
...
デプロイ完了後、本番は <https://sharebox-dls.pages.dev/> でアクセスできる。Google OAuth
...
1. 未ログイン状態で <https://sharebox-dls.pages.dev/> にアクセス → `/login`
   へリダイレクトされる
