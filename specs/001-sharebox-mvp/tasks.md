...
- [~] **T021** [P] *(撤回)* — Design Review でシングルユーザーアプリにヘッダの自分アバターは冗長と判断。Avatar コンポーネント削除。
...
- [~] **T042** [P] [UI] *(撤回)* — Design Review で本人がストレージ表示は不要と判断。spec FR-034 / SC-009 と共に撤回。
...
- [~] **T097** [P] [BE] *(撤回)* — FR-034 撤回に伴い `/api/storage/usage` エンドポイントは作らない
- [~] **T098** [P] [UI→BE 接続] *(撤回)* — StorageUsage コンポーネント自体が削除されたため
