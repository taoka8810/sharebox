...
- [ ] **T029** [P] [US1] [UI] `src/routes/login/+page.svelte` を作成: ヒーロー見出し、サブテキスト、Google でログイン (Primary Button)、デザイン確認のためのリンクは無し
- [ ] **T030** [P] [US1] [UI] `src/routes/denied/+page.svelte` を作成: アクセス拒否メッセージ、再ログインリンク (mock)
- [ ] **T031** [P] [US1] [UI] `src/lib/components/forms/TextComposer.svelte` を作成: TextArea + 文字数カウンタ (100,000 上限) + 投稿ボタン + バリデーションエラー表示 (空白のみ / 上限超過)
- [ ] **T032** [P] [US1] [UI] `src/lib/components/timeline/TextEntry.svelte` を作成: 本文プレビュー、改行保持、相対時刻、クリップボードアイコンボタン、削除アイコンボタン
- [ ] **T033** [US1] [UI] `src/routes/+page.svelte` (タイムライン) を作成し、`timeline.ts` のモックを `$state` に取り込んで TextEntry をループ表示。Composer で投稿すると mock state に追加、削除ボタンで mock state から除去 (T031, T032 に依存)

### US2 UI — File Sharing

- [ ] **T034** [P] [US2] [UI] `src/lib/components/forms/FileUploader.svelte` を作成: ドラッグ&ドロップゾーン + ファイル選択ボタン + 50MB 超過時のエラー表示 + アップロード中のモック ProgressBar (setInterval で 0→100% を演出)
- [ ] **T035** [P] [US2] [UI] `src/lib/components/timeline/FileEntry.svelte` を作成し、内部で `category` に応じて image / video / other の表示分岐
  - `image`: `<img>` インラインプレビュー (lazy)
  - `video`: `<video preload="metadata" controls>`
  - `other`: ファイル形式アイコン + ファイル名 + ファイルサイズ + 「ダウンロード」リンク
- [ ] **T036** [US2] [UI] `+page.svelte` の timeline ループに FileEntry を追加し、FileUploader からの mock 投稿で state に積めるようにする

### US3 UI — URL Sharing with OGP

- [ ] **T037** [P] [US3] [UI] `src/lib/components/forms/UrlComposer.svelte` を作成: URL Input + 投稿ボタン + バリデーションエラー (空 / 不正スキーム)
- [ ] **T038** [P] [US3] [UI] `src/lib/components/timeline/UrlEntry.svelte` を作成し、`ogp_status` に応じて 3 状態を表示
...
- [ ] **T039** [US3] [UI] `+page.svelte` の timeline ループに UrlEntry を追加し、UrlComposer からの mock 投稿で `pending → success/failed` を `setTimeout` で演出

### Common UI Across Stories

- [ ] **T040** [P] [UI] `src/lib/components/timeline/EntryFilter.svelte` を作成: タブ風 (All / Text / File / URL)、選択状態をクライアント state に保持
- [ ] **T041** [P] [UI] `src/lib/components/timeline/Timeline.svelte` (もし `+page.svelte` から切り出すなら): EntryFilter + ループ + EmptyState を統合
- [ ] **T042** [P] [UI] `src/lib/components/StorageUsage.svelte` を作成: ProgressBar + "X MB / 1 GB" 表示 (mock 488MB)
- [ ] **T043** [P] [UI] レイアウトに mock のログアウトボタンを配置 (Ghost Button、押下で `console.log` のみ)
- [ ] **T044** [UI] `+page.svelte` の上部に Composer 切替 UI (テキスト / ファイル / URL のタブまたはセグメント) を作る (T031, T034, T037 に依存)
- [ ] **T045** [UI] レスポンシブ確認: Tailwind v4 のブレークポイント (`sm`, `md`, `lg`) を使い、モバイル <600px で 1 カラム、タブレット 600-1080px で適度な余白、デスクトップ >1080px で max-width 1200px
- [ ] **T046** [UI] 全画面・全 EntryType でローディング/エラー/空状態の UI を仕込む (Skeleton、Toast、EmptyState の使用)
- [ ] **T047** [UI] クリップボードコピーは `navigator.clipboard.writeText` を使い、成功 Toast を出す
- [ ] **T048** [UI] [P] Vitest で UI コンポーネントの軽量レンダリングテストを 1〜2 件追加 (Button のバリアント切替、TextEntry の本文表示) — フェーズ A の動作確認用
...
- [ ] **T077** [US1] [BE] `src/routes/api/entries/[id]/+server.ts`: DELETE — エントリ owner 検証、`share_entry` 削除 (cascade で text_post も)、204
...
- [ ] **T086** [US2] [BE] `src/routes/api/files/[id]/+server.ts`: DELETE — 該当 `share_entry` を削除し、R2 オブジェクトも削除 (best-effort で先に R2、その後 DB)
...
- [ ] **T090** [US2] [UI→BE 接続] FileEntry の削除ボタンを `/api/files/[id]` DELETE にバインド
