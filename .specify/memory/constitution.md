...
Rationale: PATCH amendment to clarify the auth stack wording. Lucia v3 was
deprecated by its maintainer in 2025; the lucia-auth.com tutorials now recommend
using Arctic for OAuth plus a hand-rolled session layer built on @oslojs/crypto
and @oslojs/encoding. The Technology Stack table is updated to reflect the
actual library names. No principles are added, removed, or redefined.
...
Modified sections:
  - Technology Stack (Fixed Constraints) — auth row clarified
...
Templates requiring updates:
  ...
  ✅ specs/001-sharebox-mvp/plan.md — Complexity Tracking entry for the auth
     drift will be marked as resolved by this PATCH in the same commit.
